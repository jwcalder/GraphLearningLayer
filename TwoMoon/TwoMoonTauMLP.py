...
def save_plot(encoded_data, y, fname):
  ...
  fname1 = 'save/tau/{}/{}.png'.format('mlp',fname+'_Emb')
  plt.axis('off')
  plt.savefig(fname1)
  plt.close()
...
def save_loss(loss_list):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('save/tau/{}/train_loss_plot.png'.format('mlp'))
    plt.close()
...
if __name__ == '__main__':
    ...
    save_folder = 'save/tau/{}'.format('mlp')
    ...
    for epoch in np.arange(num_epochs)+1:
        ...
        if (epoch) in plot_list:
            ...
            encoded_data = model.encode(X).detach().cpu().numpy()
            fname = '{}'.format(epoch)
            save_plot(encoded_data, y, fname)
    save_loss(loss_list)
